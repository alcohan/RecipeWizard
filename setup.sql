...
SELECT 
  t.Id
  , SUM(i.Weight * t.Quantity)/r.OutputQty AS Weight
  , SUM(i.Cost * t.Quantity)/r.OutputQty AS Cost
  , SUM(i.Calories * t.Quantity)/r.OutputQty AS Calories
  , SUM(i.TTLFatGrams * t.Quantity)/r.OutputQty AS TTLFatGrams
  , SUM(i.SatFatGrams * t.Quantity)/r.OutputQty AS SatFatGrams
  , SUM(i.CholesterolMilligrams * t.Quantity)/r.OutputQty AS CholesterolMilligrams
  , SUM(i.SodiumMilligrams * t.Quantity)/r.OutputQty AS SodiumMilligrams
  , SUM(i.CarbGrams * t.Quantity)/r.OutputQty AS CarbGrams
  , SUM(i.FiberGrams * t.Quantity)/r.OutputQty AS FiberGrams
  , SUM(i.SugarGrams * t.Quantity)/r.OutputQty AS SugarGrams
  , SUM(i.ProteinGrams * t.Quantity)/r.OutputQty AS ProteinGrams

 FROM tree t
 LEFT JOIN ingredients i ON t.ChildIngredient = i.id
 LEFT JOIN Recipes r on t.Id=r.Id
 GROUP BY t.Id,r.OutputQty;
...
AS SELECT r.Name, r.Unit, r.OutputQty, rtv.* FROM Recipes r JOIN RecipeTotalValues rtv ON r.id=rtv.Id;
...
AS SELECT Id
 ...
 , CAST(Calories AS Int)
 , CAST(TTLFatGrams AS Int)
 , CAST(SatFatGrams AS Int)
 , CAST(CholesterolMilligrams AS Int)
 , CAST(SodiumMilligrams AS Int)
 , CAST(CarbGrams AS Int)
 , CAST(FiberGrams AS Int)
 , CAST(SugarGrams AS Int)
 , CAST(ProteinGrams AS Int)
